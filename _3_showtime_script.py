...
def start():
    with open('movie_data.csv') as csvfile:
        ...
        for row in readCSV:
            ...
            print('fetch', fetch)
            if len(fetch) > 0 and fetch[0]:
                movieId = str(fetch[0]['id'])
            else:
            ...
            fetch = cur.fetchall()
            if len(fetch) > 0:
                theatreId = str(fetch[0])
            else:
                # print 'Unknown theatreName - %s'%theatreName cur.execute('INSERT INTO missing_data_showtime(
                # `movie_name`,`theatre_name`,`cine_type`,`start_at`,`movie_Url`,`language`) VALUES ("' + movieName +
                # '","' + theatreName + '","Dolby","' + formatDate(date) + ' ' + formatTime(time) + '","' + link +
                # '","");')
                sqlmissing = 'INSERT INTO `missing_data_showtime` ( \
